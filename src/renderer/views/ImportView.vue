...
const parsedRetained=(file:FileState)=>[...new Set(file.retainedText.split(/[\s,;]+/).map(v=>v.trim()).filter(Boolean))];
...
<template>
  ...
  <div class="process-steps mt-4">
    ...
    <StepSection :number="3" title="Empleados retenidos" description="La lista es opcional e independiente para cada TXT." :disabled="!files.length"><details v-for="item in files" :key="`ret-${item.file.token}`"><summary><strong>{{ item.file.name }}</strong><span>{{ parsedRetained(item).length }} empleados</span></summary><textarea v-model="item.retainedText" class="form-control" rows="3" placeholder="Uno por línea o separados por coma" @input="resetRetained(item)" />
                                                                                                                                                       <ul v-if="item.retainedMatches" class="validation-results mt-2"><li v-for="match in item.retainedMatches" :key="match.employeeNumber"><span>{{ match.employeeNumber }} · {{ match.employeeName||'Sin nombre' }}</span><span>{{ match.found?`${match.matchingRecords} movimientos`:'No encontrado' }}</span></li></ul><label v-if="item.retainedMatches?.some(m=>!m.found)" class="form-check"><input v-model="item.missingAcknowledged" class="form-check-input" type="checkbox" /> Confirmo continuar con empleados no encontrados</label></details>
