...
import type { ImportGroupSummary } from '@shared/types/payroll';
...
const items = ref<ImportGroupSummary[]>([]); const total = ref(0); const loading = ref(true); const page = ref(1);
const year = ref<number | undefined>(); const error = ref(''); const openingGroupId = ref<number | null>(null); const PAGE_SIZE = 25;
function periodLabel(group: ImportGroupSummary): string { return `${group.year} · ${group.fortnights.length ? group.fortnights.map((q) => `Q${String(q).padStart(2, '0')}`).join(', ') : 'Sin quincenas'}`; }
function completedDate(value: string | null): string { return value ? new Date(value).toLocaleDateString('es-MX') : '—'; }
async function load(): Promise<void> { error.value = ''; loading.value = true; try { const response = await window.sefiplanApi.getImportGroupHistory({ page: page.value, pageSize: PAGE_SIZE, ...(year.value ? { year: year.value } : {}) }); items.value = response.items; total.value = response.total; }
  catch (cause) { error.value = errorMessage(cause, 'No se pudo consultar el histórico.'); } finally { loading.value = false; } }
async function openFolder(groupId: number): Promise<void> { openingGroupId.value = groupId; try { const opened = await window.sefiplanApi.openGroupReportFolder(groupId); if (!opened) throw new Error('El expediente todavía no tiene consolidado disponible.'); }
  catch (cause) { error.value = errorMessage(cause, 'No se pudo abrir la carpeta.'); } finally { openingGroupId.value = null; } }
onMounted(load);
</script>
<template>
  <PageHeader title="Histórico de expedientes" description="Consulta expedientes, las quincenas que contienen, sus archivos y reportes conciliados." />
  ...
  <section v-else class="history-list" aria-labelledby="history-list-title"><header class="history-list__header"><div><span class="eyebrow">Expedientes procesados</span><h2 id="history-list-title">{{ total.toLocaleString('es-MX') }} registrados</h2></div><p>Expande un expediente para consultar cada TXT.</p></header>
    <ol class="history-list__items"><li v-for="group in items" :key="group.id" class="history-record"><article><div class="history-record__summary"><div class="history-record__period"><span class="history-record__label">Ejercicio y quincenas</span><strong>{{ periodLabel(group) }}</strong></div><div class="history-record__file"><span class="history-record__label">Expediente {{ group.id }} · Versión {{ group.version }}</span><strong>{{ group.fileCount }} {{ group.fileCount === 1 ? 'archivo' : 'archivos' }} · {{ group.completedFiles }} conciliados</strong></div><div class="history-record__total"><span class="history-record__label">Total general</span><MoneyValue :cents="group.totalAmountCents" /></div><StatusBadge :status="group.status" /><button class="btn btn-outline-secondary btn-sm history-record__action" type="button" :disabled="openingGroupId !== null || group.status !== 'COMPLETED'" @click="openFolder(group.id)"><i class="bi bi-folder2-open" /> {{ openingGroupId === group.id ? 'Abriendo…' : 'Abrir reportes' }}</button></div>
      ...
      <details class="history-record__files"><summary>Ver {{ group.batches.length }} intentos de archivo</summary><div class="table-responsive"><table class="table table-sm align-middle mb-0"><thead><tr><th>Quincena</th><th>Tipo</th><th>Archivo</th><th>Intento</th><th>Estado</th><th class="text-end">Registros</th><th class="text-end">Total</th></tr></thead><tbody><tr v-for="batch in group.batches" :key="batch.id"><td>Q{{ String(batch.fortnight).padStart(2, '0') }}</td><td>{{ batch.payrollType }}</td><td class="filename-cell">{{ batch.originalFilename }}</td><td>{{ batch.attempt }}</td><td><StatusBadge :status="batch.status" /></td><td class="text-end">{{ batch.totalLines.toLocaleString('es-MX') }}</td><td class="text-end"><MoneyValue :cents="batch.totalAmountCents" /></td></tr></tbody></table></div></details>
